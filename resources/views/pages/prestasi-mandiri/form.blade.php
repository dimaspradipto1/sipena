...
<section class="section">
    ...
    <div class="card border-0 shadow-sm mb-4" style="background: linear-gradient(135deg, #eef2ff 0%, #e0e7ff 100%);">
        <div class="card-body p-4">
            <span class="badge bg-primary-subtle text-primary border border-primary-subtle px-3 py-1 mb-2">
                {{ $prestasiMandiri->exists ? 'Edit Prestasi' : 'Tambah Prestasi' }}
            </span>
    ...
    <form action="{{ $prestasiMandiri->exists ? route('prestasi-mandiri.update', $prestasiMandiri->id) : route('prestasi-mandiri.store') }}" method="POST">
        @csrf
        @if($prestasiMandiri->exists)
            @method('PUT')
        @endif
