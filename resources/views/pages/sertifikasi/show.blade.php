@extends('layouts.dashboard.template')

@section('content')
<div class="pagetitle">
    <h1>Sertifikasi</h1>
    <nav>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('dashboard.index') }}">Dashboard</a></li>
            <li class="breadcrumb-item"><a href="{{ route('sertifikasi.index') }}">Sertifikasi</a></li>
            <li class="breadcrumb-item active">Form</li>
        </ol>
    </nav>
</div><!-- End Page Title -->

<section class="section">
    <div class="card shadow-sm border-0">
        <div class="card-body p-4">
            <h5 class="card-title fw-bold text-dark mb-4 p-0">View Data</h5>

            <div class="row g-3">
                <!-- Level & Nama Sertifikasi -->
                <div class="col-md-6">
                    <label class="form-label small fw-bold text-dark mb-1">Level</label>
                    <div class="p-2 rounded bg-light border-0 text-dark small">
                        {{ $sertifikasi->level ?? '-' }}
                    </div>
                </div>

                <div class="col-md-6">
                    <label class="form-label small fw-bold text-dark mb-1">Nama Sertifikasi</label>
                    <div class="p-2 rounded bg-light border-0 text-dark small fw-bold text-primary">
                        {{ $sertifikasi->nama_sertifikasi ?? '-' }}
                    </div>
                </div>

                <!-- Nama Penyelenggara -->
                <div class="col-12">
                    <label class="form-label small fw-bold text-dark mb-1">Nama Penyelenggara</label>
                    <div class="p-2 rounded bg-light border-0 text-dark small">
                        {{ $sertifikasi->nama_penyelenggara ?? '-' }}
                    </div>
                </div>

                <!-- URL Sertifikasi -->
                <div class="col-12">
                    <label class="form-label small fw-bold text-dark mb-1">URL Sertifikasi</label>
                    <div class="p-2 rounded bg-light border-0 text-dark small text-break">
                        @if($sertifikasi->url_sertifikasi)
                            <a href="{{ $sertifikasi->url_sertifikasi }}" target="_blank" class="text-primary text-decoration-none">{{ $sertifikasi->url_sertifikasi }}</a>
                        @else
                            -
                        @endif
                    </div>
                </div>

                <!-- Link Dokumen Sertifikat & Tanggal Sertifikat -->
                <div class="col-md-6">
                    <label class="form-label small fw-bold text-dark mb-1">Link Dokumen Sertifikat</label>
                    <div class="p-2 rounded bg-light border-0 text-dark small text-break">
                        @if($sertifikasi->link_dokumen_sertifikat)
                            <a href="{{ $sertifikasi->link_dokumen_sertifikat }}" target="_blank" class="text-primary text-decoration-none">{{ $sertifikasi->link_dokumen_sertifikat }}</a>
                        @else
                            -
                        @endif
                    </div>
                </div>

                <div class="col-md-6">
                    <label class="form-label small fw-bold text-dark mb-1">Tanggal Sertifikat</label>
                    <div class="p-2 rounded bg-light border-0 text-dark small">
                        {{ $sertifikasi->tanggal_sertifikat ? $sertifikasi->tanggal_sertifikat->format('Y-m-d') : '-' }}
                    </div>
                </div>

                <!-- Link Foto Kegiatan & Link Dokumen Undangan -->
                <div class="col-md-6">
                    <label class="form-label small fw-bold text-dark mb-1">Link Foto Kegiatan</label>
                    <div class="p-2 rounded bg-light border-0 text-dark small text-break">
                        @if($sertifikasi->link_foto_kegiatan)
                            <a href="{{ $sertifikasi->link_foto_kegiatan }}" target="_blank" class="text-primary text-decoration-none">{{ $sertifikasi->link_foto_kegiatan }}</a>
                        @else
                            -
                        @endif
                    </div>
                </div>

                <div class="col-md-6">
                    <label class="form-label small fw-bold text-dark mb-1">Link Dokumen Undangan</label>
                    <div class="p-2 rounded bg-light border-0 text-dark small text-break">
                        @if($sertifikasi->link_dokumen_undangan)
                            <a href="{{ $sertifikasi->link_dokumen_undangan }}" target="_blank" class="text-primary text-decoration-none">{{ $sertifikasi->link_dokumen_undangan }}</a>
                        @else
                            -
                        @endif
                    </div>
                </div>

                <!-- Keterangan -->
                <div class="col-12">
                    <label class="form-label small fw-bold text-dark mb-1">Keterangan</label>
                    <div class="p-2 rounded bg-light border-0 text-dark small">
                        {{ $sertifikasi->keterangan ?? '-' }}
                    </div>
                </div>

                <!-- Data Mahasiswa Table -->
                <div class="col-12 mt-4">
                    <h6 class="fw-bold text-dark mb-2">Data Mahasiswa</h6>
                    <div class="table-responsive">
                        <table class="table table-bordered align-middle mb-0">
                            <thead class="bg-light">
                                <tr>
                                    <th style="width: 60px;">No</th>
                                    <th style="width: 250px;">NIM</th>
                                    <th>Nama Mahasiswa</th>
                                </tr>
                            </thead>
                            <tbody>
                                @if(!empty($sertifikasi->data_mahasiswa) && count($sertifikasi->data_mahasiswa) > 0)
                                    @foreach($sertifikasi->data_mahasiswa as $idx => $mhs)
                                        <tr>
                                            <td class="fw-bold">{{ $idx + 1 }}</td>
                                            <td>{{ $mhs['nim'] ?? '-' }}</td>
                                            <td>{{ $mhs['nama'] ?? '-' }}</td>
                                        </tr>
                                    @endforeach
                                @else
                                    <tr>
                                        <td colspan="3" class="text-center text-muted small py-3">Belum ada data mahasiswa</td>
                                    </tr>
                                @endif
                            </tbody>
                        </table>
                    </div>
                </div>

                <!-- Data Dosen Table -->
                <div class="col-12 mt-4">
                    <h6 class="fw-bold text-dark mb-2">Data Dosen</h6>
                    <div class="table-responsive">
                        <table class="table table-bordered align-middle mb-0">
                            <thead class="bg-light">
                                <tr>
                                    <th style="width: 60px;">No</th>
                                    <th style="width: 250px;">NUPTK/NIDN</th>
                                    <th>Nama Dosen</th>
                                    <th>Surat Tugas</th>
                                </tr>
                            </thead>
                            <tbody>
                                @if(!empty($sertifikasi->data_dosen) && count($sertifikasi->data_dosen) > 0)
                                    @foreach($sertifikasi->data_dosen as $idx => $dsn)
                                        <tr>
                                            <td class="fw-bold">{{ $idx + 1 }}</td>
                                            <td>{{ $dsn['nidn'] ?? '-' }}</td>
                                            <td>{{ $dsn['nama'] ?? '-' }}</td>
                                            <td>
                                                @if(!empty($dsn['url_surat']))
                                                    <a href="{{ $dsn['url_surat'] }}" target="_blank" class="text-primary text-decoration-none">{{ $dsn['url_surat'] }}</a>
                                                @else
                                                    -
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
                                @else
                                    <tr>
                                        <td colspan="4" class="text-center text-muted small py-3">Belum ada data dosen</td>
                                    </tr>
                                @endif
                            </tbody>
                        </table>
                    </div>
                </div>

                <!-- Bottom Button -->
                <div class="col-12 mt-4">
                    <a href="{{ route('sertifikasi.index') }}" class="btn text-white fw-medium px-4 shadow-sm" style="background-color: #f97316; border-color: #f97316;">
                        Cancel
                    </a>
                </div>
            </div>
        </div>
    </div>
</section>
@endsection
