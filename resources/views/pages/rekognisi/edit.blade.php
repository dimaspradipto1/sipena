...
<div class="pagetitle">
    ...
    <nav>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">Edit Rekognisi</li>
        </ol>
    </nav>
    <p class="text-muted small">Perbarui data rekognisi, mahasiswa, dan dosen pendamping.</p>
</div><!-- End Page Title -->
...
<section class="section">
    ...
    <div class="card border-0 shadow-sm mb-4" style="background: linear-gradient(135deg, #eef2ff 0%, #e0e7ff 100%);">
        <div class="card-body p-4">
            ...
            <h5 class="fw-bold text-dark mb-1">Form rekognisi terpadu</h5>
            <p class="text-secondary small mb-0">Data mahasiswa dan dosen kini diisi langsung di form ini, sehingga tidak perlu berpindah halaman setelah data rekognisi tersimpan.</p>
        </div>
    </div>
    ...
    <form action="{{ route('rekognisi.update', $rekognisi->id) }}" method="POST">
        @csrf
        @method('PUT')

        <!-- Card 1: Data Rekognisi -->
        <div class="card shadow-sm border-0 mb-4">
            <div class="card-body p-4">
                <div class="d-flex align-items-center mb-3">
                    ...
                    <div>
                        <h5 class="fw-bold text-dark mb-0">Data Rekognisi</h5>
                        <small class="text-muted">Informasi rekognisi, mitra penyelenggara, dan dokumen bukti.</small>
                    </div>
                </div>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('level') is-invalid @enderror" id="level" name="level" required>
                            <option value="" disabled {{ old('level', $rekognisi->level) ? '' : 'selected' }}>Pilih Level</option>
                            @foreach($options['levels'] as $key => $label)
                                <option value="{{ $key }}" {{ old('level', $rekognisi->level) == $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>

                    <div class="col-md-6">
                        <label for="nama_rekognisi" class="form-label fw-medium">Nama Rekognisi<span class="text-danger">*</span></label>
                        <div class="input-group">
                            <span class="input-group-text bg-light text-secondary"><i class="bi bi-box"></i></span>
                            <input type="text" class="form-control @error('nama_rekognisi') is-invalid @enderror" id="nama_rekognisi" name="nama_rekognisi" value="{{ old('nama_rekognisi', $rekognisi->nama_rekognisi) }}" placeholder="Nama Rekognisi" required>
                        </div>
                    </div>

                    <div class="col-md-6">
                        <label for="jenis" class="form-label fw-medium">Jenis<span class="text-danger">*</span></label>
                        <select class="form-select @error('jenis') is-invalid @enderror" id="jenis" name="jenis" required>
                            <option value="" disabled {{ old('jenis', $rekognisi->jenis) ? '' : 'selected' }}>Pilih Jenis</option>
                            @foreach($options['jenises'] as $key => $label)
                                <option value="{{ $key }}" {{ old('jenis', $rekognisi->jenis) == $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>

                    <div class="col-md-6">
                        <label for="nama_penyelenggara" class="form-label fw-medium">Nama Penyelenggara/Mitra<span class="text-danger">*</span></label>
                        <div class="input-group">
                            <span class="input-group-text bg-light text-secondary"><i class="bi bi-building"></i></span>
                            <input type="text" class="form-control @error('nama_penyelenggara') is-invalid @enderror" id="nama_penyelenggara" name="nama_penyelenggara" value="{{ old('nama_penyelenggara', $rekognisi->nama_penyelenggara) }}" placeholder="Penyelenggara/Mitra" required>
                        </div>
                    </div>

                    <div class="col-12">
                        <label for="url_rekognisi" class="form-label fw-medium">URL Rekognisi<span class="text-danger">*</span></label>
                        <div class="input-group">
                            <span class="input-group-text bg-light text-secondary"><i class="bi bi-link-45deg"></i></span>
                            <input type="url" class="form-control @error('url_rekognisi') is-invalid @enderror" id="url_rekognisi" name="url_rekognisi" value="{{ old('url_rekognisi', $rekognisi->url_rekognisi) }}" placeholder="URL Rekognisi">
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            <span class="input-group-text bg-light text-secondary"><i class="bi bi-link-45deg"></i></span>
                            <input type="url" class="form-control @error('link_dokumen_sertifikat') is-invalid @enderror" id="link_dokumen_sertifikat" name="link_dokumen_sertifikat" value="{{ old('link_dokumen_sertifikat', $rekognisi->link_dokumen_sertifikat) }}" placeholder="URL Sertifikat">
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            ...
                            <input type="date" class="form-control @error('tanggal_sertifikat') is-invalid @enderror" id="tanggal_sertifikat" name="tanggal_sertifikat" value="{{ old('tanggal_sertifikat', $rekognisi->tanggal_sertifikat ? $rekognisi->tanggal_sertifikat->format('Y-m-d') : '') }}">
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            <span class="input-group-text bg-light text-secondary"><i class="bi bi-link-45deg"></i></span>
                            <input type="url" class="form-control @error('link_foto_kegiatan') is-invalid @enderror" id="link_foto_kegiatan" name="link_foto_kegiatan" value="{{ old('link_foto_kegiatan', $rekognisi->link_foto_kegiatan) }}" placeholder="URL Foto Kegiatan">
                        </div>
                    </div>

                    <div class="col-md-6">
                        <label for="link_dokumen_undangan" class="form-label fw-medium">Link Dokumen/Undangan/Bukti Lain</label>
                        <div class="input-group">
                            <span class="input-group-text bg-light text-secondary"><i class="bi bi-link-45deg"></i></span>
                            <input type="url" class="form-control @error('link_dokumen_undangan') is-invalid @enderror" id="link_dokumen_undangan" name="link_dokumen_undangan" value="{{ old('link_dokumen_undangan', $rekognisi->link_dokumen_undangan) }}" placeholder="URL Dokumen">
                        </div>
                    </div>
                    ...
                    <div class="col-12">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control @error('keterangan') is-invalid @enderror" id="keterangan" name="keterangan" value="{{ old('keterangan', $rekognisi->keterangan) }}" placeholder="Keterangan tambahan">
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="card shadow-sm border-0 mb-4">
            <div class="card-body p-4">
                ...
                <div id="container-mahasiswa">
                    @php
                        $mahasiswaList = old('data_mahasiswa', $rekognisi->data_mahasiswa ?? [['nim' => '', 'nama' => '']]);
        ...
        <div class="card shadow-sm border-0 mb-4">
            <div class="card-body p-4">
                ...
                <div id="container-dosen">
                    @php
                        $dosenList = old('data_dosen', $rekognisi->data_dosen ?? [['nidn' => '', 'nama' => '', 'url_surat' => '']]);
                        if (empty($dosenList)) {
        ...
        <div class="d-flex justify-content-between align-items-center mb-5">
            <a href="{{ route('rekognisi.index') }}" class="btn btn-light border px-4 shadow-sm">
