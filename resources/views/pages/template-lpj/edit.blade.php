...
<div class="pagetitle">
    ...
    <p class="text-muted small">Perbarui informasi tautan atau berkas template LPJ.</p>
</div><!-- End Page Title -->
...
<section class="section">
    ...
    <div class="card border-0 shadow-sm mb-4" style="background: linear-gradient(135deg, #eef2ff 0%, #e0e7ff 100%);">
        <div class="card-body p-4">
            ...
            <h5 class="fw-bold text-dark mb-1">Form Edit Template LPJ</h5>
            <p class="text-secondary small mb-0">Perbarui tautan download kustom atau ganti berkas dokumen resmi (.docx / .pdf).</p>
        </div>
    </div>
    ...
    <form action="{{ route('template-lpj.update', $templateLpj->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            ...
            <div class="card-body p-4">
                <div class="d-flex align-items-center mb-4">
                    ...
                    <div>
                        ...
                        <small class="text-muted">Perbarui informasi template di bawah ini.</small>
                    </div>
                </div>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-8">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control @error('nama') is-invalid @enderror" id="nama" name="nama" value="{{ old('nama', $templateLpj->nama) }}" placeholder="Nama Template LPJ" required>
                        </div>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select @error('kategori') is-invalid @enderror" id="kategori" name="kategori" required>
                            @foreach($kategoriList as $key => $label)
                                <option value="{{ $key }}" {{ old('kategori', $templateLpj->kategori) == $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-12">
                        ...
                        <div class="d-flex gap-4 p-3 bg-light rounded-3 border">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="tipe" id="tipe_link" value="link" {{ old('tipe', $templateLpj->tipe) === 'link' ? 'checked' : '' }}>
                            ...
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="tipe" id="tipe_file" value="file" {{ old('tipe', $templateLpj->tipe) === 'file' ? 'checked' : '' }}>
                    ...
                    <div class="col-12" id="section-link">
                        ...
                        <div class="input-group">
                            ...
                            <input type="url" class="form-control @error('url_link') is-invalid @enderror" id="url_link" name="url_link" value="{{ old('url_link', $templateLpj->url_link) }}" placeholder="https://drive.google.com/file/d/...">
                        </div>
                    ...
                    <div class="col-12 d-none" id="section-file">
                        <label for="file_document" class="form-label fw-medium">Unggah Berkas Dokumen</label>
                        @if($templateLpj->file_path)
                            <div class="p-2 mb-2 bg-light rounded border d-flex align-items-center justify-content-between">
                                <span class="small text-secondary">
                                    <i class="bi bi-file-earmark-check text-success me-1"></i> File saat ini: <strong>{{ $templateLpj->file_name ?? basename($templateLpj->file_path) }}</strong>
                                </span>
                                <a href="{{ route('template-lpj.download', $templateLpj->id) }}" class="btn btn-sm btn-outline-primary" target="_blank">
                        ...
                        <small class="text-muted">Biarkan kosong jika tidak ingin mengganti berkas yang sudah ada. Format didukung: .docx, .doc, .pdf, .odt, .zip (Maksimal 20 MB).</small>
                    </div>

                    <!-- Keterangan -->
                    <div class="col-12">
                        <label for="keterangan" class="form-label fw-medium">Keterangan / Petunjuk Pengisian</label>
                        <textarea class="form-control @error('keterangan') is-invalid @enderror" id="keterangan" name="keterangan" rows="3" placeholder="Tuliskan catatan panduan atau instruksi penggunaan template ini...">{{ old('keterangan', $templateLpj->keterangan) }}</textarea>
                    </div>

                    <!-- Status Aktif Checkbox -->
                    <div class="col-12">
                        <div class="form-check form-switch p-3 bg-light rounded-3 border">
                            <input class="form-check-input ms-0 me-3" type="checkbox" role="switch" id="is_active" name="is_active" value="1" {{ old('is_active', $templateLpj->is_active) ? 'checked' : '' }}>
        ...
        <div class="d-flex justify-content-between align-items-center mb-5">
            ...
            <button type="submit" class="btn btn-dark px-4 shadow-sm" style="background-color: #0f172a; border-color: #0f172a;">
                <i class="bi bi-save me-1"></i> Perbarui Template
            </button>
        </div>
    </form>
</section>
